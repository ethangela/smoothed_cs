...
if attck_iter:
    if sample:
        model_name = f'SMT_smp{sample}_std{std}_ADV_itr{attck_iter}_alp{alpha}_eps{epsilon}_ISTA_Net_pp'
    else:
        model_name = f'ADV_itr{attck_iter}_alp{alpha}_eps{epsilon}_ISTA_Net_pp'
else:
...
for epoch_i in range(start_epoch + 1, end_epoch + 1):
    ...
    for idx, data in enumerate(rand_loader):
        ...
        if attck_iter:
            ...
            for i in range(attck_iter):
                #smooth
                if sample:
                    for j in range(sample):
                        batch_n = torch.normal(0, std, size=batch_x.shape).type(torch.FloatTensor).to(device) #(1, 1, 264, 264)
